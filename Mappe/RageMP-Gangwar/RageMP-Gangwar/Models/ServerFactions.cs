...
namespace RageMP_Gangwar.Models
{
    class ServerFactions
    {
        ...
        public static void GetAllFactionInformations(Client player)
        {
            try
            {
                ...
                {
                    var skip = i * 5;
                    player.TriggerEvent("Client:TeamBrowser:fillInformations", JsonConvert.SerializeObject(items.Skip(skip).Take(5).ToList()));
                }

                if (rest != 0) player.TriggerEvent("Client:TeamBrowser:fillInformations", JsonConvert.SerializeObject(items.Skip((int)iterations * 5).ToList()));
            }
            catch(Exception e)
            {
                Console.WriteLine($"{e}");
        ...
        public static void GetAllFactionVehicles(Client player)
        {
            try
            {
                ...
                {
                    var skip = i * 10;
                    player.TriggerEvent("Client:Garage:setVehicles", JsonConvert.SerializeObject(items.Skip(skip).Take(10).ToList()));
                }

                if (rest != 0) player.TriggerEvent("Client:Garage:setVehicles", JsonConvert.SerializeObject(items.Skip((int)iterations * 10).ToList()));
            }
            catch(Exception e)
            {
                Console.WriteLine($"{e}");
