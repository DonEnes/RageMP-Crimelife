    ...
    {       
        ...
        public void CMD_Kick(Client player, Client target, string reason)
        {
            try
            {
                ...
                NAPI.Chat.SendChatMessageToAll($"[~r~Vace~w~] {player.Name} hat {target.Name} vom Server gekickt. Grund: {reason}");
                target.Kick($"{reason}");
        ...
        public void CMD_Ban(Client player, Client target, string reason)
        {
            ...
            NAPI.Chat.SendChatMessageToAll($"[~r~Vace~w~] {player.Name} hat {target.Name} gebannt. Grund: {reason}");
            Models.ServerAccounts.SetPlayerBanned(tID, true, false);
            target.Kick($"{reason}");
        ...
        public void CMD_Timeban(Client player, Client target, int hours, string reason)
        {
            ...
            NAPI.Chat.SendChatMessageToAll($"[~r~Vace~w~] {player.Name} hat {target.Name} temporär gebannt. Grund: {reason}");
        ...
        public void CMD_SetXP(Client player, int multiplikator)
        {
            try
            {
                ...
                NAPI.Chat.SendChatMessageToAll($"[~r~Vace~w~] Der XP-Multiplikator wurde von {player.Name} auf {multiplikator}x gesetzt.");
            }
            catch (Exception e)
            {
                Console.WriteLine($"{e}");
        ...
        public void CMD_Unban(Client player, int accId)
        {
            try
            {
                ...
                NAPI.Chat.SendChatMessageToPlayer(player, $"[~r~Vace~w~] Spieler wurde erfolgreich entbannt (ID: {accId}).");
