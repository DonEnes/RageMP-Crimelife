...
namespace RageMP_Gangwar.Handler
{
    ...
    {
        public static void openBrowser(Client player, ServerFactionsGarage factionGarage)
        {
			try
			{
				...
				if (pID <= 0) return;
				player.TriggerEvent("Client:Garage:clearInfo");
				...
				player.TriggerEvent("Client:Garage:openBrowser", factionGarage.id);
			}
			catch (Exception e)
			{
				Console.WriteLine($"{e}");
		...
		public void Garage_TakeVeh(Client player, string displayName, int type, int garageId)
		{
			try
			{
					...
					case 1:
						{
							...
							{
								NAPI.Chat.SendChatMessageToPlayer(player, $"[~r~Vace~w~] Für dieses Fahrzeug reicht dein Level nicht aus.");
								return;
							}
							break;
						}
					...
					case 3:
						{
							...
							{
								NAPI.Chat.SendChatMessageToPlayer(player, $"[~pc~Vace~w~] Für dieses Fahrzeug reicht dein Prestige-Level nicht aus.");
								return;
							}
							break;
						}
