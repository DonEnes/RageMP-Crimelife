    ...
    class DeathHandler : Script
    {
        ...
        public void OnPlayerDeath(Client player, Client killer, uint reason)
        {
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    player.TriggerEvent("Client:HUD:RespawnEvent");
                ...
                if(player.getcurrentDuellPartner() != 0)
                {
                    ...
                    tKiller.TriggerEvent("Client:HUD:KillEvent", false, tKiller.Name, ServerAccounts.GetPlayerKills(tKiller.getAccountId()), ServerAccounts.GetPlayerDeaths(tKiller.getAccountId()), player.Name, false);
                    player.TriggerEvent("Client:HUD:DeathEvent", false, killer.Name, ServerAccounts.GetPlayerKills(playerId), ServerAccounts.GetPlayerDeaths(playerId), player.Name);
                ...
                if (player.getcurrentDuellPartner() == killerId && killer.getcurrentDuellPartner() == playerId)
                {
                    ...
                    ServerAccounts.SetPlayerDuellPartner(playerId, 0);
                    ServerAccounts.SetPlayerDuellPartner(killerId, 0);
                    killer.TriggerEvent("Client:HUD:KillEvent", false, killer.Name, ServerAccounts.GetPlayerKills(killerId), ServerAccounts.GetPlayerDeaths(killerId), player.Name, false);
                    player.TriggerEvent("Client:HUD:DeathEvent", false, killer.Name, ServerAccounts.GetPlayerKills(playerId), ServerAccounts.GetPlayerDeaths(playerId), player.Name);
                    TeamHandler.SpawnPlayer(killer);
                    return;
                }
                ...
                if (ServerAccounts.GetPlayerFFAArena(playerId) > 0 && ServerAccounts.GetPlayerFFAArena(killerId) > 0 && ServerAccounts.GetPlayerFFAArena(playerId) == ServerAccounts.GetPlayerFFAArena(killerId))
                {
                    ...
                    if (ServerAccounts.GetPlayerEXP(killerId) >= ServerAccounts.GetPlayerLevel(killerId) * 15)
                    {
                        ...
                        killer.TriggerEvent("Client:HUD:LevelUP", ServerAccounts.GetPlayerLevel(killerId), ServerAccounts.GetPlayerLevel(killerId) * 15);
                    }
                    killer.TriggerEvent("Client:HUD:KillEvent", false, killer.Name, ServerAccounts.GetPlayerKills(killerId), ServerAccounts.GetPlayerDeaths(killerId), player.Name, false);
                    player.TriggerEvent("Client:HUD:DeathEvent", false, killer.Name, ServerAccounts.GetPlayerKills(playerId), ServerAccounts.GetPlayerDeaths(playerId), player.Name);
                } 
                else
                {
                    ...
                    killer.TriggerEvent("Client:HUD:KillEvent", sameTeam, killer.Name, ServerAccounts.GetPlayerKills(killerId), ServerAccounts.GetPlayerDeaths(killerId), player.Name, false);
                    player.TriggerEvent("Client:HUD:DeathEvent", sameTeam, killer.Name, ServerAccounts.GetPlayerKills(playerId), ServerAccounts.GetPlayerDeaths(playerId), player.Name);
                }

                killer.TriggerEvent("Client:HUD:setExperience", ServerAccounts.GetPlayerEXP(killerId));
                killer.TriggerEvent("Client:HUD:setKills", ServerAccounts.GetPlayerKills(killerId));
                player.TriggerEvent("Client:HUD:setDeaths", ServerAccounts.GetPlayerDeaths(playerId));
