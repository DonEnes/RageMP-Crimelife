...
namespace RageMP_Gangwar.Handler
{
    ...
    {
        [Command("1vs1")]
        public void CMD_Duell(Client player, Client target)
        {
            try
            {
                ...
                if(Models.ServerAccounts.GetPlayerFFAArena(pID) != 0 || Models.ServerAccounts.GetPlayerFFAArena(tID) != 0) { NAPI.Chat.SendChatMessageToPlayer(player, $"[~r~Vace~w~] Du oder die ausgewählte Person sind in einer FFA Arena."); return; }
                if(player.getcurrentDuellPartner() != 0 || target.getcurrentDuellPartner() != 0) { NAPI.Chat.SendChatMessageToPlayer(player, $"[~r~Vace~w~] Du oder die ausgewählte Person sind bereits in einem anderen Duell."); return; }
        ...
        [Command("disable1vs1")]
        public void DisableDuell(Client player)
        {
            try
            {
        ...
        [Command("accept1vs1")]
        public void CMD_DuellAccept(Client target, Client player)
        {
            try
            {
                if (player == null || !player.Exists || !player.hasAccountId() || target == null || !target.Exists || !target.hasAccountId()) return;
