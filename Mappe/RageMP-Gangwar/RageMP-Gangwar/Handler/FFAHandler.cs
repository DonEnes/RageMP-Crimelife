    ...
    public class FFAHandler : Script
    {
        ...
        public void Event_FFA(Client player, string arenaName)
		{
			try
			{
				...
				{
					player.SendChatMessage($"[~p~Vace~w~] Die FFA Arena ist ~p~voll.~w~ Wähle eine andere.");
					return;
				}
				ServerAccounts.SetPlayerFFAArena(pID, arenaData.id);
				ServerFFA.IncreaseFFAPlayer(arenaData.id);
				TeamHandler.SpawnPlayer(player);
				player.SendChatMessage($"[~p~Vace~w~] Willkommen in der FFA Arena. Benutze ~p~/quitffa~w~ um die FFA-Arena zu verlassen.");
			}
			catch (Exception e)
			{
				Console.WriteLine($"{e}");
