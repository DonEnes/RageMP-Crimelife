    ...
    class Commands : Script
    {
        ...
        public void resetCMD(Client player)
        {
            try
            {
                ...
                player.SendChatMessage($"[~r~Vace~w~] K/D zurückgesetzt.");
            }
            catch (Exception e)
            {
                Console.WriteLine($"{e}");
        ...
        public void CMD_Invite(Client player, Client inviteTarget)
        {
            try
            {
                ...
                {
                    NAPI.Chat.SendChatMessageToPlayer(player, $"[~r~Vace~w~] Du bist in keiner privaten Fraktion oder bist nicht der Leader.");
                    return;
                }

                if(ServerAccounts.GetAccountFaction(tID) != 0)
                {
                    NAPI.Chat.SendChatMessageToPlayer(player, $"[~r~Vace~w~] Der Spieler {inviteTarget.Name} ist bereits in einer privaten Fraktion.");
                    return;
                }
        ...
        public void CMD_acceptInvite(Client player, Client inviter)
        {
            try
            {
                ...
                {
                    NAPI.Chat.SendChatMessageToPlayer(player, $"[~r~Vace~w~] Du bist bereits in einer privaten Fraktion.");
                    return;
                }
        ...
        public void CMD_Uninvite(Client player, Client target)
        {
            try
            {
                ...
                {
                    NAPI.Chat.SendChatMessageToPlayer(player, $"[~r~Vace~w~] Der Spieler {target.Name} ist nicht in deiner Fraktion.");
                    return;
                }
                string factionName = ServerFactions.GetFactionName(pFaction);
